...
**Tradeoff.** The single 30-second window is the weakness: with only one window analyzed, level 3 cannot recover from a slice that happens to be unrepresentative (an intro, a breakdown, or a section at a different tempo), and it accepts a low-confidence answer rather than also analyzing longer windows. Because levels 3-5 are identical today, choosing 4 or 5 over 3 adds no accuracy; the higher numbers are a statement of intent, not a guarantee of a better result, until the ML tiers are wired.
