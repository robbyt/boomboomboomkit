...
---
**What it does.** Runs the _optimal_ technique set (ACF sharpening plus sub-band octave voting plus fine-grid refinement, three candidates) over a single 30-second window, with no progressive retry. Adding ACF sharpening to the baseline is the best single DSP technique on the OA300 corpus (worth about two tracks over the baseline), so level 3 is the entry point to the empirically best DSP configuration. Levels 3, 4, and 5 currently resolve to this identical DSP setup; the finer granularity is a reserved control for future intensity-proportional compute budgeting, since the ComputeBudget fractions are uniform (_budget_ is _ComputeBudget.default_) at every level today.
