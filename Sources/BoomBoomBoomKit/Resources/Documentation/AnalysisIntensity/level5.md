...
**When to pick it.** Choose it when you want the best single-window DSP accuracy and want to signal the highest intent within the no-retry band. It is functionally identical to levels 3 and 4 today. It is DSP-only, so every _EnsemblePolicy_ is coherent (ML-inert unless a technique is wired up) with _dspOnly_ the default. If a track's tempo may drift across its length, move up to level 6, where progressive analysis with a second 60-second window begins.
