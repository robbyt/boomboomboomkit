...
**When to pick it.** Choose it when accuracy matters more than the last few milliseconds and a track may not be uniform — the second, longer window gives the estimate a wider view than a single 30-second slice, and merging across windows can correct a reading the first window got wrong. It is a strong general-purpose choice just below the full level 7. It is DSP-only; every _EnsemblePolicy_ is coherent (ML-inert without a wired technique) and _dspOnly_ is the natural pairing.

**Tradeoff.** Running two windows costs roughly twice the single-window compute, and merging does not by itself cure an octave error: if the octave-doubled tempo self-reports high confidence in both windows, the default maximum-confidence merge still selects the wrong value, because more windows agreeing on the same mistake does not make it right. Level 6 also tops out at a 60-second window, so a track whose representative groove only appears later still benefits from level 7's third 90-second window.
