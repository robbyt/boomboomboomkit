...
---
**What it does.** Squares the autocorrelation function element-wise before peak selection, which sharpens the periodicity peaks so the dominant tempo separates from its neighbors. It is a single vector pass over a few thousand values and changes only how pronounced the ACF peaks are, not the underlying onset detection. It produces the largest observed single-technique accuracy gain in the library's internal ablation, and it is part of the _optimal_ preset that drives the default intensity mapping.
