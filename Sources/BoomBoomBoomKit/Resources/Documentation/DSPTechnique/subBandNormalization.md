...
**When to pick it.** Use it when one frequency band is drowning out rhythmically important onsets in others — a very loud kick masking snare and hat patterns, for instance — so that the combined envelope reflects the full rhythmic picture rather than just the loudest drum. It is part of the _dnbOptimized_ preset, where balancing sub-bands helps on bass-heavy material, and it is a reasonable addition whenever spectral balance rather than transient timing is the limiting factor.

**Tradeoff.** On the OA300 corpus the technique is neutral — it changes no track's outcome relative to baseline — so it adds computation without a measured accuracy gain on the library's primary material, and enabling it is a bet that your corpus differs from the tuning set. Normalization can also amplify noise in quiet bands: a sub-band with no real rhythmic content but a small spurious peak gets scaled up to full weight, letting incidental high-frequency hiss contribute to the onset envelope as if it were a genuine, evenly-matched drum voice.
