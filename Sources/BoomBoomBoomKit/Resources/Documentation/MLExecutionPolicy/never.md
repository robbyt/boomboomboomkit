...
**When to pick it.** Choose it when you want a guaranteed DSP-only run for reproducibility, latency, or as a control arm — the same intent as _EnsemblePolicy.dspOnly_ but expressed on the execution axis. It is the right setting when a model is present in the configuration for other reasons but you specifically want it dormant, or when you are measuring the DSP baseline without any inference cost.

**Tradeoff.** Never running ML forgoes any accuracy a model could add: on genres or edge cases where DSP is weak and a trained model would recover the tempo, _never_ ships the DSP mistake with no second opinion. Because this policy is not yet Options-wired, selecting it today is also a statement of intent rather than an active gate — the current pipeline's ML execution is governed elsewhere, so a consumer expecting _never_ to change present-day behavior will find it inert until the wiring lands.
