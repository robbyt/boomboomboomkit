...
**Tradeoff.** Running ML unconditionally pays the inference cost on every track, including the many where DSP was already correct and the model adds nothing, which is pure overhead. Worse, a systematically miscalibrated model that always runs influences every result under a fusing ensemble policy, so its bias contaminates tracks DSP would have gotten right. Because the policy is not yet Options-wired, selecting _always_ today does not force inference, so a consumer expecting it to change present-day behavior finds it inert until the wiring lands.
