...
prose; here we compare and flag the sharp edges.

## EnsemblePolicy

``EnsemblePolicy`` has five cases. It is **not** a `String`/`CaseIterable` enum —
``EnsemblePolicy/weightedVoting(_:)`` carries an associated ``SignalWeights`` — so
it exposes `allPolicies` and a hand-written stable key instead.
...
> ``EnsemblePolicy/dspOnly`` — so out of the box, no ML is invoked and output is
