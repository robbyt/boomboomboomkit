...
def run_check(docs_root: Path, output_dir: Path) -> int:
    """Verify-only (drift gate for `make docc-validate`): assert the on-disk
    `Cases/` directory is exactly what a fresh generation would produce.
    ...
    page's bytes must equal the freshly-generated bytes. Catches a stale
    `Cases/` (source changed, not regenerated), a hand-edited page, a deleted
    directory, or an orphan file. Any malformed source, empty corpus, or on-disk
    drift exits nonzero.
    """
    reject_path_overlap(docs_root, output_dir)
...
def parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="Verify-only: assert the on-disk Cases/ matches a fresh generation, write nothing.",
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        if args.check:
            ...
            print(f"docc-transclude --check: {count} pages consistent with Cases/")
        else:
