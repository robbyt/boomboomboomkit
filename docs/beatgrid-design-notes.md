...
- **Priming-trim descope + `offset(by:)` (resolved post-8.5 review, 2026-06-14).** The epic's "playback-aligned by subtracting `leadingTrimFrames`" AC was NOT implemented and the descope is final: `AVAudioFile`/`ExtAudioFile` *already* trims declared `mPrimingFrames` before `PCMBufferReader` sees sample 0 (M4A/AAC, MP3 w/ LAME-Xing, CAF), so the decoded stream is already playback-aligned and a library-side priming subtraction would **double-correct** (~48 ms the wrong way for AAC). The ~48 ms in §6/README is therefore a worst-case *upper bound* for a hypothetical non-trimming decoder, not Hilbert's expected offset (near-zero through AVFoundation). For the offsets that ARE real on Hilbert's side — output-device latency, a manual nudge, an exotic headerless-stream residual — apply them with the non-destructive `BeatGrid.offset(by:)` (shifts beats + downbeats + `gridOrigin` uniformly, clamp-the-delta so spacing holds). Hilbert sources that value from its own runtime (`AVAudioEngine.outputLatency` / user control); it must NOT subtract codec priming itself. See `8-5-pressure-release.md`.
