...
> **Epic B outcome (added 2026-07-26).** The replacement did not happen. Epic 7 delivered the corpus, the harnesses and three retrained models, then closed **BYOW** on 2026-06-09: the v2 model fails both FR-18 bundle gates by wide margins (OA300 43/82 against `> 55/82`; GiantSteps 348/661 against `≥ 537/661`) and no model bundles. FR-16's octave-aware loss shipped and the model still octave-doubles, which places the defect below the loss function. FR-24 and FR-25 were never run. The FRs below are retained as written; each affected FR carries a dated amendment with the measured numbers. Follow-up is the Epic 12 charter in `epics.md`, which is charter-only.
...
**FR-18 — Trained-model evaluation gates.** Before any decision to bundle the model in the SPM library on the public `main` branch (i.e., ship the compiled `.mlmodelc` inside the library's resources so consumers get it by default), the trained model must clear: (a) all 4 named DnB triplet targets correct, (b) OA300 Acc1 strictly exceeds the DSP single-window ceiling of 55/82, (c) GiantSteps Acc1 ≥ 537/661 floor, (d) confidence-calibration floor (see FR-25). If any gate fails, the model ships as a BYOW reference only — published in `_bmad-output/ml-models/` for developers to wire up via `BNNSTechnique(modelURL:)`. *(**Status amended 2026-07-26, recording the Epic 7 close of 2026-06-09: both load-bearing gates were measured and both failed. The decision was BYOW.** The sentence above is retained as the original conditional; this is its resolution. The authoritative run was `maskedMelPretrain` seed 42 at `featureSetVersion` v2, tempo-band rebalanced, trained on the 1,534-track corpus (Strong tier 523) after the KDD-B4 signoff. Evaluated through the production Swift runtime at `.mlOnly`: OA300 Acc1 **43/82 (52.4%)** against the `> 55/82` gate (b), and GiantSteps Acc1 **348/661 (52.6%)** against the `≥ 537/661` gate (c). Gate (a) — the four named DnB sentinels — and gate (d) — the FR-25 calibration floor — were **never evaluated**, because the decision graph is monotone-downgrade-only: once (b) and (c) both fail, the remaining two can confirm the outcome but never reverse it. Three retrains established that the failure is **structural, not data-volume**: OA300 50 → 48 → 43 and GiantSteps 296 → 330 → 348 across the baseline, +90 and +190 hand-label batches, with the sub-120 BPM band flat at 2/95 throughout. The +190 batch bought 120-140 BPM (+32 tracks) by cannibalizing 140-160 (−16) and 175+ (−2), dragging OA300 below its own baseline. One measurement caveat, stated because it cuts toward the model rather than against it: the ML Acc1 figures above were scored at 4% tolerance, while the 55/82 and 537/661 reference points are DSP results at 2%. The looser standard was applied to ML and it still finished 13 tracks below the OA300 bar (which requires 56 to strictly exceed 55) and 189 below the GiantSteps bar. **Consequence:** no compiled model ships in `Sources/BoomBoomBoomKitML/`; consumers wire their own via `BNNSTechnique(modelURL:)`. `epic7CloseDecision="byow"` is stamped into the seed-42 model metadata and training log; git tag `epic-7-close`. Two close conditions were waived deliberately — single-seed rather than the 42/43/44 matrix (dispersion only disambiguates a result **near** the gate, and this is not near), and FR-24/FR-25 not run (see their amendments below). Follow-up is the **Epic 12 charter** — "Model-quality redesign: octave-aware BPM model" in `epics.md` — which is charter-only; no Story 12.x exists. Evidence: `_bmad-output/implementation-artifacts/epic-7-retro-2026-06-05.md` close-out addendum; `_bmad-output/ml-training/v2-runs/_archive/maskedMelPretrain-seed42-191tracks-rebalance/FR18-RESULT.md`.)*
...
**FR-24 — Semi-supervised expansion must prove net benefit.** Secondary supervised and pseudo-labeled data (FR-13) may only be promoted into the active training set if an ablation against supervised-only training improves or preserves all acceptance gates — OA300 Acc1, GiantSteps Acc1, DnB challenge set, labeled validation accuracy — within documented tolerance. Regression beyond tolerance blocks promotion. *(**Status amended 2026-07-26: this gate never ran.** FR-24 requires a matched two-arm comparison — `maskedMelPretrain` against `supervisedAugmented` on the same corpus and split. At the Epic 7 close no such pair existed: the `supervisedAugmented` checkpoint on disk was a stale earlier snapshot (corpusHash `2294f44a` against the authoritative run's `25e59a18`, with a differing splitHash), so the ablation was **structurally un-runnable**, not skipped for convenience. It was not reconstructed because the decision graph only downgrades — running it could have confirmed the BYOW outcome but never promoted the model. Consequence: no semi-supervised expansion was promoted into an active training set on this evidence, and whether expansion pays off on this corpus is **open, not answered**. Any Epic 12 retrain that wants to claim FR-24 must build the matched pair first. Evidence: `epic-7-retro-2026-06-05.md` close-out addendum, waived condition 2.)*

**FR-25 — Confidence calibration verified.** Trained-model confidence scores are calibrated to predictive correctness — higher confidence corresponds to higher accuracy. Acceptance includes a measurable calibration metric (e.g., expected calibration error, softmax distribution shape) and a documented floor that the previously-bundled `giantsteps_v1.mlmodelc` failed to meet (the forensic anchor: `softmax_max_p95 = 0.294`, bimodal at 125/175 regardless of input). The specific calibration metric and numeric floor are committed in the training/eval plan before Epic B is considered complete. Calibration failure blocks bundling regardless of Acc1. *(**Status amended 2026-07-26: neither half of this FR was delivered.** The specific calibration metric and its numeric floor were to be "committed in the training/eval plan before Epic B is considered complete" — they never were, and Epic B closed anyway on 2026-06-09. The verification itself (`make calibration-verify`) was never run against the v2 model. Story 7-7 shipped the harness; the measurement it exists to produce does not exist. Two consequences follow. First, FR-18 gate (d) is **unevaluated**, so an Epic 12 re-run of FR-18 starts with that gate still undefined. Second, the calibration failure mode that killed `giantsteps_v1.mlmodelc` (`softmax_max_p95 = 0.294`, bimodal at 125/175 regardless of input) has **not been ruled out** for the v2 model. "Calibration failure blocks bundling" was never load-bearing at the close only because FR-18's accuracy gates failed first and nothing is bundled. Evidence: `epic-7-retro-2026-06-05.md` close-out addendum, waived condition 3.)*
...
**FR-29 — Long-file sync stability.** Beat timestamps remain audibly accurate at the end of long files (5+ minute tracks). The specific drift tolerance (numeric, in ms-per-minute or beats-over-duration) is committed in the acceptance corpus spec before Epic C is considered complete. *(**Status amended 2026-07-25: this FR is UNVALIDATED, and the sentence above is retained as the original aspiration, not as a current guarantee.** Nothing shipped establishes end-of-file accuracy against audio. What was committed in Story 8.7 was a P95 last-beat drift tolerance of ≤ 30 ms, **measured at 1,652 ms** (median 176 ms) over the gated subset of n=450 **constant-tempo** tracks ≥ 5 min drawn from the Rekordbox-derived corpus. The gate in force is now a 2.0 s regression ceiling, not the 30 ms aspiration. Critically, that metric measures how far the last raw `beats` entry strays from the library's own anchor-plus-tempo extrapolation of the same beat index. It is a grid-internal divergence between two views the library emits, computed without reference comparison, so it neither confirms nor refutes this FR's "audibly accurate" prose. The nearest against-reference evidence is FR-34's F-measure of 0.37 over the 910-track constant-tempo subset, which is corpus-wide beat agreement rather than an end-of-file measure. Automatic refinement (Stories 8.9/8.10) did not close the gap, and the epic pivoted to manual hand-correction levers (8.11/8.12). Follow-up is charter-only in Epic 13 (DSP experiments); nothing is scheduled. `MODEL_CARD.md` carries the consumer-facing disclosure.)*
...
**FR-34 — Beat-grid acceptance corpus.** Beat-grid accuracy is validated against the OA300 corpus (where DAW oracle ground truth is available) and a stratified DnB subset. Specific accuracy thresholds (F-measure floor, beat-position tolerance) live in the training/eval plan and are committed there before Epic C is considered complete. *(Amended 2026-07-25, recording the Epic 8 accept-as-shipped close of 2026-07-01: the thresholds were committed in Story 8.7 and then missed. Measured against a Rekordbox-derived JAMS oracle — 1,264 tracks, 910 constant-tempo, ±70 ms, octave-normalized — beat-position F-measure over the 910-track constant-tempo subset was **0.37** against a 0.75 AC. The downbeat detector fired on 54 of 1,264 tracks (4.3%); across the 42 constant-tempo tracks where it fired, mean octave-tolerant downbeat F-measure was **0.14** against a 0.65 AC. Those two downbeat figures have different denominators and must not be composed into a "14% of the 4%" success rate. The committed regression floors are F ≥ 0.33 and downbeat ≥ 0.10, set at the measured level as nets rather than as claims. The corpus framing above was also superseded during execution: the OA300 `.dawproject` source was operator-hand-warped, so Story 8.7 DD-13 redirected to the Rekordbox corpus for authoritative bar phase, accepting an auto-analyzed position reference. `MODEL_CARD.md` carries the consumer-facing disclosure. Follow-up is charter-only in Epic 13 (DSP experiments; Epic 12 is an ML BPM-model redesign and touches beat-grid only as an adjacent arbiter idea); nothing is scheduled. Original wording retained above.)*
...
**Calibration failure.** The previously-bundled `giantsteps_v1.mlmodelc` failed at calibration (softmax_max_p95=0.294, bimodal at 125/175 regardless of input). Mitigation: FR-25 codifies a calibration metric and floor; calibration failure blocks bundling regardless of Acc1. *(**Amended 2026-07-26: the named mitigation did not fire.** FR-25's metric and floor were never committed and the verification never ran against the v2 model (see the FR-25 amendment above). This risk is therefore **unretired, not mitigated**. It did not bite in practice only because the v2 model failed FR-18's accuracy gates first and nothing is bundled — which is an accident of ordering, not a control.)*
...
**Long-file sync drift.** Beat-grid timestamps may diverge audibly from audio over 5+ minute tracks. Mitigation: FR-29 (long-file sync stability); FR-34 acceptance corpus includes long-file representative tracks. *(**Unmitigated and unverified** — amended 2026-07-25. Stated carefully, because the obvious stronger claim is not supported: no shipped measurement tests divergence-from-audio on long files, so this risk has NOT been observed to materialize as written. What is measured is (a) a grid-internal divergence of 1,652 ms P95 between the raw `beats` array and the anchor-plus-tempo extrapolation on constant-tempo tracks ≥ 5 min, against a 30 ms committed tolerance, and (b) corpus-wide against-reference beat agreement of F 0.37 against 0.75. Neither is an end-of-file audio comparison. The mitigations named above therefore did not close the risk; they did not measure it. The shipped response is consumer guidance plus manual correction primitives (`BeatGridTempoLock.bpm`, downbeat-strategy selection, manual anchor reposition), not automatic precision. See the FR-29 and FR-34 amendments above.)*
