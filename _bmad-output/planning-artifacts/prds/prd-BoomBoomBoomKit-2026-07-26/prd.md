...
- ~~**Q5.** How does a style prior get its style? User-declared, metadata-derived, or classified — each has a different failure surface.~~ ~~**RESOLVED 2026-07-28 (operator): classified.** The prior derives style from a classifier rather than a caller-declared value or a file tag. This buys coverage with no caller input and no dependence on absent or wrong genre tags, and accepts a router-error failure surface plus a second model in the deployment path — the exact cost the 2026-06-09 roundtable cited when ranking genre-MoE last. **Consequence: FR-54 now carries a classifier dependency it did not have when written, and that deployment objection now applies to F1.** Recorded rather than smoothed over.~~ **REVERSED 2026-08-05 by Story 12.2 (`_bmad-output/implementation-artifacts/12-2-style-classifier-decision.md`). This reverses an operator decision, recorded 2026-07-28.** The answer is now **caller-declared**, not classified, and the style-conditioned prior of FR-54 is not delivered. Basis for the reversal: §5.1 FR-54a, written after Q5, names it as one of two available exits ("revisit Q5 in favour of the caller-declared option, which needs no model"), and the **product-policy premise** recorded in §1.1 of the decision artifact — automatic style inference is outside this library's mission, and callers own their own domain constraints — is what converts an available exit into a taken one. The premise does the work. ~~and the caller-declared option now exists in shipped code as `Options.tempoScanRange` and `Options.perceptualWindow` (Story 12.1, 2026-08-02), where in July 2026 it did not.~~ **CORRECTED 2026-08-06 (external review): that clause is the same conflation the decision artifact struck at its §7 (`12-2-style-classifier-decision.md:513-515`), and it is false.** Q5's caller-declared option is a caller-declared **style**; Story 12.1 shipped caller-declared BPM **bounds**, a different axis, and no caller-declared style prior was built. FR-53's `Options.tempoScanRange` and `Options.perceptualWindow` are the **alternative octave mechanism Epic 12 ships instead of FR-54** — the replacement, not the warrant. The router-error failure surface and the second model in the deployment path, which Q5 accepted as costs, are not incurred. Q5's original resolution is retained above as the record of what was decided and why. The reversal path is section 8 of the decision artifact.
...
| **AS-7** | The SMC 2015 style-prior result transfers to our corpus and pipeline. **TESTED 2026-08-06 AS A HARD SEARCH-RANGE BOUND, AND NEGATIVE — but NOT resolved for the published mechanism** (`_bmad-output/implementation-artifacts/12-2-smc-prior-replication.md`). Story 12.2's evidence run moved `Options.tempoScanRange` to the published `130...180` over three corpora at otherwise-shipped defaults. Drum-and-bass Acc1: OA300 58 to 54, GiantSteps 466 to 361 octave-strict (537 to 426 floor-compatible), FR-14-conformant Tony 510 to 492. All-tier Tony is +44, the single positive number, and it is carried entirely by the 312 Marginal-tier rows FR-14 excludes from training. Non-DnB cost under the same configuration is large: GiantSteps non-DnB Acc1 355 to 251. **Three limits on what this settles, all load-bearing.** (1) It is a hard-filter *analogue*: the repository's only source for SMC's prior is the phrase "DnB prior 130-180 BPM", which fixes a range but not whether it was applied as a search constraint or a candidate reweighting (`epics.md:2079` records the same caution), so **soft reweighting remains untested and AS-7 is not refuted**. (2) It does not refute SMC 2015 — their baseline had a drum-and-bass octave catastrophe (7.19% Acc1) this pipeline does not have, and our GiantSteps DnB baseline already meets or exceeds their post-prior 78.42%. (3) Tolerance is not like-for-like: ours 2%, theirs 4%. ~~**STILL UNTESTED as of 2026-08-05 (Story 12.2). ~~WEAKENED~~ was claimed earlier the same day and is withdrawn on external review.**~~ The 2026-08-05 reasoning below is retained as the record of why the Story 12.1 report was not a test. The withdrawn claim rested on treating `_bmad-output/implementation-artifacts/12-1-tempo-range-impact-report.json` as "the first direct test" of the published result. **It is not a test of style-prior transfer at all.** It inferred no style, accepted no declared style, and conditioned nothing by style: one global window was applied to all 82 tracks regardless of genre. It also moved `Options.perceptualWindow`, the octave-*normalization* window, rather than `Options.tempoScanRange`, the pair that sizes the candidate *search* (the report's own `metric` field reads "only perceptualWindow moved"), and moved it to `100...200` rather than the published `130-180` (§4.3, `prd.md:101` as of 2026-08-05). ~~**The published configuration has never been run on this pipeline.**~~ **SUPERSEDED 2026-08-06: the published range has now been run as a hard `tempoScanRange` bound; see the verdict at the head of this row.** The report's figures, kept here because they are what the earlier verdict cited, are overall `deltaAcc1 0` / `deltaAcc2 -3`, and `drum-and-bass` at n=67, `deltaAcc1 -1` / `deltaAcc2 -2`. They are evidence about globally narrowing the octave-fold window, which is a different assumption from the one AS-7 states. Full reasoning at `_bmad-output/implementation-artifacts/12-2-style-classifier-decision.md` §6.1 | F1 loses its evidence base and drops down the ranking. **Partially realized, and not by measurement:** F1's classifier half is rejected outright by Story 12.2, on a product-policy premise rather than on evidence against AS-7. Its range half shipped as FR-53. ~~The assumption itself is untouched and the experiment that would settle it, `tempoScanRange` at `130...180` over the drum-and-bass rows, has not been run~~ **UPDATED 2026-08-06: that experiment has now been run and is negative, so F1's evidence base is measurably weaker as a hard bound. It is not eliminated: the soft-reweighting form FR-54 actually specifies is still untested, and it is the only remaining path by which this lever could pay** |
