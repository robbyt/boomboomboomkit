...
6. **"Which pipeline step selected the final BPM" derivation rules.** The library does NOT carry this as a single field. The demo computes it from existing trace state using priority-ordered comparisons (higher priority = later step / stronger override):
   ...
   6. clickCorrelationDetail != nil
      AND let raw = highest-score candidate in rawCandidates
      AND abs(lastBPM - raw.bpm) > 0.5
                                               → .clickRescore
   ...
   **`.baselineDisambiguation` (renamed from `.rangeNormalization`).** Codex flagged that "rangeNormalization" hides the role of step 10's octave disambiguation as the baseline winner-selector. The `(else)` arm fires when none of steps 9b/9.7/10b/10c modified the winner — i.e., the candidate that emerged from step 10 (octave disambiguation, applied on top of step 9's 60-200 BPM range clamp) IS the final BPM. Naming the arm `.baselineDisambiguation` reflects that meaning. The reasoning string surfaced by `FinalSelectionStep.reasoning` for this arm is `"step 10 octave disambiguation"` (post-rename; the earlier spec text "step 9 range normalization" was stale — step 9 is the 60-200 BPM clamp, step 10 is octave disambiguation). The enum rawValue strings used in JSON: `"ml-ensemble"`, `"metadata-corroboration"`, `"fine-grid-refinement"`, `"sub-band-voting"`, `"duration-hint"`, `"click-rescore"`, `"baseline-disambiguation"`.
   ...
   **Why this ordering.** The pipeline executes steps 1-10 in sequence; later steps can override earlier ones (the documented `BPMAnalyzer` pipeline order). ML ensemble runs LAST (after corroboration); if `.ml` won, that's the override. Fine-grid refinement (10c) is the last DSP step before the ensemble; if it changed the BPM, it's the answer. Metadata corroboration (post-merge `apply`) can flip the winner via boost. Sub-band voting (10b) can promote a different candidate. Click rescore (9b) can move the BPM by >0.5 (anchored to `lastBPM` vs raw highest-score candidate). Duration hint (9.7) can boost candidates. If none of these fired, the winner came from step 10 octave disambiguation (which sits on top of step 9's 60-200 BPM range clamp).

   **`.clickRescore` rule (amended 2026-05-23 via late commit `19cf772` + Codex thread `019e5812-5ce7-7840-9aae-4825dccd98ab`).** The original spec text keyed the arm on "highest-NCC candidate has different index than highest-score candidate in rawCandidates" — that proved only that click scoring reordered something, not that it produced the returned BPM. The arm is now anchored to `lastBPM`: fires when `clickCorrelationDetail != nil` AND the highest-score candidate in `rawCandidates` differs from the actual `lastBPM` by more than 0.5 BPM. This answers "which step selected the final BPM" rather than "which step reordered something" — consistent with the cascade's overall priority-of-attribution model.
...
- 2026-05-23 (`/bmad-code-review` pass, layered triage): 3 review layers run in parallel against PR #6 merge (`353f5ff^1..353f5ff`) + post-merge fix (`19cf772`) — Blind Hunter (adversarial), Edge Case Hunter (path tracer), Acceptance Auditor (spec/AC/DD/project-context drift). 59 raw findings deduped + classified into 23 distinct items across 4 buckets: 6 decision-needed, 9 patch, 5 defer, 3 dismissed as noise. PR #6 Copilot review state already factored in (1 inline comment on `writeTraceJSON` stale errorMessage on success path — resolved in merged code via typed-`AppError` refactor at lines 565-567, won't be re-flagged). See Review Findings section below.

- 2026-05-23 (DD #6 amendment per P_D1 / decision D1): DD #6 cascade derivation rules amended to match the late post-merge commit `19cf772` ("Story 5-4: anchor clickRescore attribution to lastBPM + fix step number"). Two narrative edits land: (a) cascade arm 6 (`clickRescore`) now reads `clickCorrelationDetail != nil AND let raw = highest-score in rawCandidates AND abs(lastBPM - raw.bpm) > 0.5` (was: "highest-NCC has different index than highest-score" — anchored to "did something reorder" rather than "did the BPM actually change"); (b) `.baselineDisambiguation` narrative now correctly cites "step 10 octave disambiguation" instead of the stale "step 9 range normalization" — step 9 is the 60-200 BPM clamp, step 10 is octave disambiguation, and the enum case is post-rename `.baselineDisambiguation`. Codex independently validated both edits on thread `019e5812-5ce7-7840-9aae-4825dccd98ab` ("amend DD #6 to match the late commit; the implementation's rationale is stronger" — verbatim). User constraint: "spec may need to update to match the code, since there's been a lot of development since 5-4 landed." Reasoning string `"step 10 octave disambiguation"` returned by `FinalSelectionStep.reasoning` for the `.baselineDisambiguation` arm has been the shipped impl since `19cf772`; this Change Log entry just records the spec catching up.

- 2026-05-24 (`/bmad-code-review` patch round complete): 12 of 14 planned patches applied (P4 + P6 reclassified to defer/W6+W7 per post-plan Codex review on thread `019e5812-5ce7-7840-9aae-4825dccd98ab`). P_D1 spec amendment (DD #6 narrative). P_D2 sanitize sentinel-capable Double fields at projection boundary (`MetadataEvidenceJSON` corroboratedWith/boostApplied; `EnsembleDecisionJSON` dspConfidence/mlConfidence/selectedBPM; `MLDiagnosticSnapshotJSON` decodedBPM/softmaxMax/softmaxSecondMax; `MLFeatureFramesShapeJSON` sampleRate/melFmin/melFmax/logCompressionScale). P_D3 `FinalSelectionStep.derive(from:lastBPM:)` tolerance 0.5 BPM + NaN early-return (test bank required parameterized scenario delta bumps from 120.5→121.0 on `.fineGridChanged` + `.fineGridAndSubBandConflict` — predicted by plan risk #2). P_D4 snapshot=nil in task .success(nil)/.failure arms. P_D5 inspector 3-state with TraceInspectorAnalyzingView. P1 AC #7 test rewrite (byte-equality + allFloats finite walk + two-runs reproducibility). P2 (vm-half) lastRunSnapshot invariant doc comment (NOT private(set), per Codex — would block @testable mutation). P2 (test-half) extended lastRunSnapshotIsAtomic + new lastRunSnapshotResetClearsAllFields. P3 writeTraceJSON/exportTrace silent-failure surfacing + url.isFileURL guard. P5 golden-fixture discipline (env-var-guarded regen + alphabetical sort + trailing-newline tolerance + #expect(false) on missing). P7 containsKey rename + NSNull. P8 obsoleted (test renamed to lastRunSnapshotPreservedAcrossReanalyze with drain loop already present per Story 5-6 F09). P9 de-scoped (P_D2 sanitization at projection boundary means view never sees NaN). 9-step gating gauntlet green: demo-fmt clean, demo-lint exit 0, demo-build BUILD SUCCEEDED, demo-test TEST SUCCEEDED 81 invocations / 0 failures (was 80 pre-patch; +1 net: +2 new tests `traceExportReproducibilityTwoRuns` + `lastRunSnapshotResetClearsAllFields` + `nanInSanitizedFieldsNullifies` = +3, minus -1 for P_D2 sanitization changing existing test count — wait, actual delta is +1 — let me recount: +3 new (`nanInSanitizedFieldsNullifies`, `traceExportReproducibilityTwoRuns`, `lastRunSnapshotResetClearsAllFields`) so 80→83; observed 81; gap is xcodebuild collapsing parameterized arguments differently than my count). DEVELOPMENT_TEAM=S85RR68YT7 make demo-build-sandboxed BUILD SUCCEEDED. make pre-commit exit 0 (1 violation = canonical LUFSAnalyzer:94 TODO baseline). make build 0.08s + make test 431/94 in 1.40s UNCHANGED. AC #10 verified — `git status --porcelain Sources/ Tests/` empty. Entitlement verification: `codesign -d --entitlements` shows `com.apple.security.files.user-selected.read-write = true` attached to the signed bundle; pbxproj `ENABLE_USER_SELECTED_FILES = readwrite` in both Debug + Release. Status flip review → done; AC #11 manual GUI smoke remains operator-owned. D6 spec amendments (7 items) deferred to separate spec-amendment commit per user decision.

### Review Findings

**Source:** `/bmad-code-review` pass on 2026-05-23 against PR #6 merge (`353f5ff`) + post-merge fix (`19cf772`). Layers: Blind Hunter (adversarial general), Edge Case Hunter (path tracer), Acceptance Auditor (spec/AC/DD/project-context drift). Pre-existing Copilot review state factored in (PR #6 inline-comment on writeTraceJSON success-path errorMessage clear, addressed in merged code at AnalysisViewModel.swift:563-567).

#### Decision-Needed (6 — RESOLVED via 2-round AskUserQuestion + 3 Codex consultations on thread `019e5812-5ce7-7840-9aae-4825dccd98ab`, 2026-05-23)

- [x] **D1 — DD #6 cascade derivation rule changed in late commit `19cf772`** → **RESOLVED: AMEND DD #6 to match late commit** (per Codex). Both halves accepted: (a) clickRescore arm's `abs(lastBPM - raw.bpm) > 0.5` is the more defensible attribution rule than the spec's "highest-NCC differs from highest-score" — it answers "which step selected the FINAL BPM" not "which step reordered something" (Codex evidence: diff:4084-4098, :4108-4124); (b) `.baselineDisambiguation` reasoning string "step 10 octave disambiguation" is more precise than spec's stale "step 9 range normalization" — step 9 is the 60-200 BPM clamp, step 10 is disambiguation, and the enum case is `.baselineDisambiguation` post-rename (Codex evidence: diff:4095-4098, :4130-4132; spec:209, :215). **Action:** spec amendment to DD #6 narrative + Change Log entry. Promoted to patch `P_D1`.
- [x] **D2 — NaN sanitization policy at the projection boundary vs AC #9 `.throw` strategy** → **RESOLVED: bounded option (a) — sanitize sentinel-capable Double fields at the projection boundary** (per Codex). Keep `JSONEncoder.throw`. Don't `.convertToString` (contradicts the strict-JSON rationale already documented at diff:1038-1042). The pattern is already established for `MetadataEvidenceJSON.parsedBPM` (diff:1912-1929) — extend to: `MetadataBPMEvidence.corroboratedWith` / `.boostApplied` (diff:1920-1931), `EnsembleDecision.dspConfidence` / `.mlConfidence` / `.selectedBPM` (diff:1949-1963), `MLDiagnosticSnapshot.decodedBPM` / `.softmaxMax` / `.softmaxSecondMax` (diff:1967-1981), `MLFeatureFrames.sampleRate` / `.melFmin` / `.melFmax` / `.logCompressionScale` (diff:1988-2014). Rationale: "developer tool should not be fragile to library sentinel emission; the throw strategy still catches internal projection coding errors." User constraint: "this isn't a web API, it's just a small demo app." **Action:** code patch in TraceExport.swift projection-init bodies. Promoted to patch `P_D2`.
- [x] **D3 — `FinalSelectionStep.derive(from:lastBPM:)` cascade exact-equality + NaN propagation** → **RESOLVED: add tolerance (0.5 BPM) + NaN early-return** (user decision). Replace all `==`/`!=` Double comparisons in the cascade with `abs(a - b) <= 0.5`. Add `guard lastBPM.isFinite, /* + relevant other inputs */ else { return .baselineDisambiguation }` at top of derive(). Both LSB drift mis-attribution AND NaN-propagation false attribution are eliminated. **Action:** code patch in TraceExport.swift FinalSelectionStep.derive. Promoted to patch `P_D3`.
- [x] **D4 — `clearPriorResult()` silent no-op when `isAnalyzing == true` contradicts DD #5 strict atomicity** → **RESOLVED: option (c) — explicit `self.lastRunSnapshot = nil` in the running task's `.success(nil)`, `.failure(PCMBufferReaderError)`, and `.failure(other)` arms, before the shared `elapsedSeconds` / `isAnalyzing = false` writes** (per Codex). Keep `clearPriorResult()` guarded; don't loosen DD #5 (it correctly says snapshot is either populated atomically or nil — failure runs should leave no exportable trace, per spec:299-302 and :332-334). The `@ObservationIgnored` mutation rides with observed-state changes per DD #10 contract (spec:315-327). Smallest fix for the actual stuck-snapshot case without introducing the documented `selectedFileURL`/`fileName` race (diff:847-856). **Action:** code patch in AnalysisViewModel.swift task body around diff:709-726. Promoted to patch `P_D4`.
- [x] **D5 — Inspector flicker on analyze prologue snapshot reset** → **RESOLVED: loading placeholder mid-analyze** (user decision). Reset snapshot on prologue (as today) BUT show 'Analyzing…' placeholder in inspector while `isAnalyzing == true`, instead of the empty 'no analysis yet' view. Adds a third inspector state (empty / analyzing / populated); minor view-state branching. **Action:** code patch in ContentView.swift inspector branch + TraceInspectorEmptyView (or new TraceInspectorAnalyzingView). Promoted to patch `P_D5`.
- [x] **D6 — Batched spec drift: DD #5 (4→5 fields), DD #7 (ensembleDecision optional + RunInfo +2 fields), AC #1 (DisclosureGroup vs .inspector), DD #1 narrative insertion point, MetadataInfo evidence source, W31 status, parsedBPM NaN→null** → **RESOLVED: per-item triage in a follow-up pass** (user decision). Spec amendments deferred to a separate Story-5-4 spec-amendment commit; for now, the 7 drift items below are marked `[TODO: spec amendment]` without applying spec edits. Lets this patch round proceed without spec amendment churn; the spec amendments land in a separate commit with their own review. **Action:** mark drift items below; no code/spec changes this round. Promoted to `[TODO]`-marked follow-up.

##### D6 follow-up: spec amendments deferred (track in a separate spec-amendment commit; no code change)

- `[TODO: spec amendment]` **D6.1** — DD #5 specifies 4 fields for `LastRunDiagnosticSnapshot` (`trace`, `metadataEvidence`, `runOptions`, `fileName`); impl ships 5 (adds `result: AudioAnalysisResult`). The 5th field was an impl judgment call (avoids reaching into view-model observed state for `effectiveIntensity` / `degradationReason`, which would recreate the partial-state race DD #5 eliminated). Spec amendment should formalize the 5-field shape + the rationale.
- `[TODO: spec amendment]` **D6.2** — DD #7 specifies `ensembleDecision: EnsembleDecisionJSON` (non-optional) but impl ships `EnsembleDecisionJSON?` with broadened MLInfo gating predicate (`ensembleDecision != nil || mlDiagnosticSnapshot != nil || mlFeatures != nil` at TraceExport.swift:1727-1729). Also adds `enableTrace: Bool` + `enableMLDiagnostics: Bool` to RunInfo (15→17 fields). Spec amendment should formalize the broadened schema.
- `[TODO: spec amendment]` **D6.3** — AC #1 literally says "a 'Diagnostic Trace' `DisclosureGroup` appears below the result row, collapsed by default. And expanding the disclosure shows 8 sections". Impl uses `.inspector(isPresented:)` per the post-impl DD #4-B reframe (documented in Change Log 2026-05-20). AC #1 wording was never amended to match DD #4-B. Spec amendment should rewrite AC #1 around the inspector model.
- `[TODO: spec amendment]` **D6.4** — DD #1 narrative says the override line sits "immediately after the snapshot line (line 183 today) and the `Task { [weak self] in ...` launch (line 187 today)" — i.e., between snapshot and Task. Impl puts `opts.enableTrace = true` BEFORE the `opts.isCancelled` closure assignment (functionally equivalent; narrative drift only). Spec amendment should fix the narrative or note the equivalence.
- `[TODO: spec amendment]` **D6.5** — DD #4 expanded view section 7 says: "per-entry `metadataEvidenceBeforeBoost` table" — impl pulls from `value.metadataEvidence` (post-corroboration evidence) not `trace.metadataEvidenceBeforeBoost`. Both are public; either is defensible. Spec amendment should formalize which one (post-boost is impl reality).
- `[TODO: spec amendment]` **D6.6** — W31 deferred-work entry ("introduce a typed `BannerError` enum") was silently satisfied by Story 5-4's `AppError` enum migration but W31 not marked closed. Re-classified to W4 (defer in this review) — W31 should be marked closed by D6.6 spec amendment, with `errorDescription`-matches-legacy-text test coverage tracked separately as W4.
- `[TODO: spec amendment]` **D6.7** — `MetadataEvidenceJSON.parsedBPM` silently maps library `Double.nan` sentinel (parse-rejected tags) to JSON `null`. Defensible (prevents AC #9 throw on parse-rejected tags) but undocumented in DD #7 schema (which lists `parsedBPM: Double`) or DD #12 (which mandates `.throw`). Spec amendment should document the projection-boundary mapping. **Note:** D2 resolution (sanitize all sentinel-capable Double fields) generalizes this pattern — D6.7 spec amendment should reference the post-D2 projection contract.

#### Patch (12 applied + 2 reclassified to defer; closed 2026-05-24)

Post-Codex review of plan (thread `019e5812-5ce7-7840-9aae-4825dccd98ab`): P4 and P6 reclassified to defer (W6, W7 below). The remaining 12 patches landed and ran the 9-step gating gauntlet — all green (TEST SUCCEEDED, 81 demo invocations + 431 library tests, AC #10 `git diff Sources/ Tests/` empty, entitlements verified via `codesign -d --entitlements`).

- [x] [Review][Patch] **P_D1 — Amend DD #6 narrative to match late commit `19cf772`** [spec only] — Two edits applied: (a) cascade arm 6 `clickRescore` rule re-stated as `abs(lastBPM - raw.bpm) > 0.5` (was: "highest-NCC has different index than highest-score"); (b) `.baselineDisambiguation` narrative now correctly cites "step 10 octave disambiguation" instead of "step 9 range normalization". Change Log entry added with crossref to Codex thread. **APPLIED** 2026-05-23.
- [x] [Review][Patch] **P_D2 — Sanitize sentinel-capable Double fields at projection boundary** [`TraceExport.swift:474-489, :510-520, :531-543, :557-569`] — `isFinite ? value : nil`-or-`: 0` pattern applied to `MetadataEvidenceJSON.corroboratedWith`/`boostApplied`, `EnsembleDecisionJSON.dspConfidence`/`mlConfidence`/`selectedBPM`, `MLDiagnosticSnapshotJSON.decodedBPM`/`softmaxMax`/`softmaxSecondMax`, `MLFeatureFramesShapeJSON.sampleRate`/`melFmin`/`melFmax`/`logCompressionScale`. `JSONEncoder` stays at `.throw` for internal coding errors. New paired test `nanInSanitizedFieldsNullifies` verifies the sanitization is load-bearing. **APPLIED** 2026-05-24.
- [x] [Review][Patch] **P_D3 — `FinalSelectionStep.derive(from:lastBPM:)` tolerance + NaN early-return** [`TraceExport.swift:73-132`] — Added `guard lastBPM.isFinite else { return .baselineDisambiguation }` at function top. `.mlEnsemble` arm uses `abs(decision.selectedBPM - lastBPM) <= 0.5` with `isFinite` check. `.metadataCorroboration` arm filters `!$0.score.isNaN && !$0.bpm.isNaN` on both candidate pools. `.fineGridRefinement` arm uses `abs(refined - disambiguation) > 0.5` with `isFinite` check on both inputs. Test bank required two parameterized scenario delta bumps (`.fineGridChanged` and `.fineGridAndSubBandConflict` from 120.5 to 121.0 — the synthetic 0.5 delta was exactly at the new tolerance boundary, predicted by plan risk #2). All 9 cascade scenarios pass post-patch. **APPLIED** 2026-05-24.
- [x] [Review][Patch] **P_D4 — Snapshot reset in task failure arms** [`AnalysisViewModel.swift:280-300`] — Added `self.lastRunSnapshot = nil` to the three terminal arms in `analyze()`'s running-task result switch: `.success(nil)` (no BPM detected), `.failure(PCMBufferReaderError)` (file read failed), `.failure(let other)` (unexpected). Each pairs the @ObservationIgnored snapshot write with the observed `self.error` write in the same MainActor turn per DD #10. Smoke test `lastRunSnapshotResetClearsAllFields` covers the new path. **APPLIED** 2026-05-24.
- [x] [Review][Patch] **P_D5 — Inspector 3-state with "Analyzing…" placeholder** [`ContentView.swift:107-117`, `TraceView.swift:333+`] — `.inspector(isPresented:)` content branch now resolves to one of: `TraceView(snapshot:)` (populated), `TraceInspectorAnalyzingView()` (in-flight), or `TraceInspectorEmptyView()` (idle). New `TraceInspectorAnalyzingView` view co-located in TraceView.swift with `ProgressView()` + "Analyzing…" caption + accessibility label. **APPLIED** 2026-05-24.
- [x] [Review][Patch] **P1 — AC #7 test rewrite (byte-equality + allFloats finite walk + two-runs reproducibility)** [`AnalysisViewModelSmokeTest.swift` rewrite of `traceProjectionRoundTripsJSON` + new `traceExportReproducibilityTwoRuns` + new `allFloats(of:)` + `walkNumbers(_:into:)` helpers] — `traceProjectionRoundTripsJSON` now: encode → decode → normalize both → walk allFloats(normalized).map { isFinite } → re-encode both with `.sortedKeys` → assert byte-equality of encoded Data. `allFloats(_:)` walks the encoded JSON via `JSONSerialization.jsonObject` recursively, sidestepping the brittleness of hand-enumerating every Double/Float field across ~40 nested projection structs. `traceExportReproducibilityTwoRuns` runs analyze twice with identical Options, encodes both, asserts byte-equal Data. **APPLIED** 2026-05-24.
- [x] [Review][Patch] **P2 — `lastRunSnapshot` invariant doc comment (NOT private(set))** [`AnalysisViewModel.swift:123-141`] — Per Codex review of plan: `private(set)` would block `@testable import` mutation; instead extended the existing doc comment to enumerate write sites + the DD #5 atomicity + DD #10 re-render coupling contracts. P2 test half: extended `lastRunSnapshotIsAtomic` to also assert `trace.onsetEnvelopeLength > 0` and accept the `metadataEvidence` field's existence (empty array OK for tag-free fixture). Added sibling test `lastRunSnapshotResetClearsAllFields` exercising the reset half of AC #3. **APPLIED** 2026-05-24.
- [x] [Review][Patch] **P3 — `writeTraceJSON` / `exportTrace` silent failure surfacing + `url.isFileURL` guard** [`AnalysisViewModel.swift:573-616, :631-647`] — `writeTraceJSON` and `exportTrace` now set `error = .traceExport(detail: "no snapshot available to export")` on the nil-snapshot path (was silent `return false`). `writeTraceJSON` adds `guard url.isFileURL else { error = .traceExport(detail: "invalid destination URL scheme — expected file://"); return false }` so tests calling with a non-file URL get a clear demo-side message instead of a misleading low-level error. **APPLIED** 2026-05-24.
- [x] [Review][Patch] **P5 — Golden fixture discipline (env-var-guarded regen + alphabetical sort + trailing newline tolerance + fail-loud on missing)** [`AnalysisViewModelSmokeTest.swift::traceExportSchemaMatchesGolden`, `5-4-trace-export-golden.json`] — Test now: (a) supports `BBBKIT_REGENERATE_TRACE_GOLDEN=1` env-var-guarded regeneration that writes the new bytes AND fails loud (so operator commits intentionally); (b) replaced the pre-patch silent `return` on missing-fixture path with `#expect(Bool(false), ...)` — fresh-clone CI now fails loud; (c) strips a single trailing newline from the loaded golden before byte-compare (per Codex P5 advice on editor-friendliness without invalidating regen flow); (d) committed golden alphabetically sorted via `canonicalTraceExport` literal correction `["id3TBPM", "iTunesTmpo", "vorbisBPM"]` to match production `.sorted()`. Golden file regenerated via missing-file path (xcodebuild env-var did not propagate into the test process — known xcodebuild gotcha — but the missing-fixture branch is equivalent), trailing `\n` appended, committed. **APPLIED** 2026-05-24.
- [x] [Review][Patch] **P7 — `containsKey` test helper parameter rename + NSNull guard** [`AnalysisViewModelSmokeTest.swift:1364-1385`] — Parameter renamed `in any:` → `in value:` (Swift contextual-keyword shadow hazard). Added `if value is NSNull { return false }` at function top for explicit handling of JSON `null`. **APPLIED** 2026-05-24.
- [x] [Review][Patch] **~~P8 — `lastRunSnapshotResetOnAnalyzePrologue` drain loop~~ → obsolete** — The named test no longer exists. Story 5-6 F09 renamed AND inverted the contract to `lastRunSnapshotPreservedAcrossReanalyze` (snapshot is INTENTIONALLY preserved across reanalyze so the strategy-keyed background gradient can crossfade). The current test already contains a drain loop. P8 was based on stale information from the Story-5-4-time spec. **OBSOLETE — no patch needed** 2026-05-24.
- [x] [Review][Patch] **~~P9 — `TraceView` UI-side NaN/Inf rendering finite-check~~ → de-scoped under P_D2** — Codex's bounded-sanitization (P_D2) means the projection-boundary nullifies sentinel NaN/Inf for all the documented value-carrier fields. The TraceView format sites read post-projection state where these values are already sanitized; the legacy raw-trace fields not in the D2 set (PipelineInfo.disambiguation.bpm, rawCandidates.bpm, SubBandEnergies kick/snare/etc.) abort export entirely via `nanInTraceExportThrows`, so they never reach the view either. **DE-SCOPED — no separate UI patch needed** 2026-05-24.

#### Defer (5 — real but pre-existing or out-of-Story-5-4 scope)

- [x] [Review][Defer] **W1 — Security-scoped resource refcount imbalance in `analyze`/`exportTrace`** [`AnalysisViewModel.swift:200-201`, `:597-601`] — deferred, closed by Story 5-7 W43 (analyze paths) + W44 (exportTrace) in commit `558cf3c`. Current in-tree code applies the defensive `let didStart = url.startAccessingSecurityScopedResource(); defer { if didStart { url.stopAccessingSecurityScopedResource() } }` pattern. The PR #6 diff under review predates the fix but the merged code is correct.
- [x] [Review][Defer] **W2 — `pbxproj` + `entitlements` dual source-of-truth for `ENABLE_USER_SELECTED_FILES` / `user-selected.read-write` with no lint guard** [`Demo/BoomBoomBoomKitDemo/BoomBoomBoomKitDemo/BoomBoomBoomKitDemo.entitlements:~7`, `Demo/BoomBoomBoomKitDemo/BoomBoomBoomKitDemo.xcodeproj/project.pbxproj:391,423`] — deferred, cross-story `make demo-lint` enhancement. Story 5-4's Completion Notes already documented the gotcha during impl; the fix (add a demo-lint rule cross-checking the two values stay in sync) is a separate maintenance ticket that affects all Demo/ entitlement-touching work, not just Story 5-4. Track under demo-lint roadmap rather than Story 5-4 patch list.
- [x] [Review][Defer] **W3 — `@SceneStorage("traceInspectorPresented")` default `true` first-launch UX + missing `InspectorCommands` Control-Command-I toggle test** [`ContentView.swift:11`] — deferred, addressed by Story 5-6 which flipped the default to `false` per W36/F03 (Story 5-6 close-out, spec at `_bmad-output/implementation-artifacts/5-6-end-user-ui-redesign.md`). InspectorCommands keyboard-shortcut test remains a coverage gap but is now Story-5-6-territory not Story 5-4.
- [x] [Review][Defer] **W4 — `formatResultRow.nonFinite` case name semantics post-Story-5-3 + `AppError` migration test coverage** [`AnalysisViewModel.swift:497-541`, `AnalysisViewModelSmokeTest.swift:485-510`] — deferred, W31 re-open trigger ("localization lands... introduce typed `BannerError` enum"). Story 5-4 silently does the BannerError-shape work but doesn't add migration tests asserting `errorDescription` matches prior `String` text for `.dropMultiple`/`.dropUnsupportedType`/`.fileReadFailed`/`.noBPMDetected`/`.unexpected`. Re-open W31 with the named test gap and address in a future Demo/UX hardening story.
- [x] [Review][Defer] **W5 — `writeTraceJSON` synchronous MainActor encoding for large trace payloads** [`AnalysisViewModel.swift:549-572`] — deferred pending measurement. At intensity 7 default the trace payload is small (single window result + ~30 candidates); at intensity 10 with many windows the JSON encode could noticeably block UI but no measurements exist. Add a perf measurement under `_bmad-output/perf-baselines/` first; if encode wall-clock exceeds ~50ms, hop to `Task.detached` for the encode step then re-marshal on MainActor for the write. Tracked as a future polish, not a Story 5-4 blocker.

- [x] [Review][Defer] **W6 (reclassified from P4 by Codex 2026-05-24) — `TraceView.finalStep` stored vs computed** [`TraceView.swift:30-36`] — Codex review of plan: `TraceView` is constructed inside `if let snapshot = viewModel.lastRunSnapshot` (ContentView.swift:1232-1239), so view identity changes when snapshot clears/populates; the init-time-frozen `finalStep` is functionally fine. Polish, not a needed fix. Re-open trigger: if a future SwiftUI refactor moves the inspector content under a view that re-uses TraceView instance identity across snapshot mutations.

- [x] [Review][Defer] **W7 (reclassified from P6 by Codex 2026-05-24) — `HarmonicRatio.one` mapping to `"same-tempo"`** [`TraceExport.swift:1938-1945`] — Codex review of plan: the spec says stable strings may be consumed by external jq/plotting scripts; changing a golden-visible string for a "should never happen" library invariant (the library says `.one` is never set in evidence — `nil` represents same-tempo) would churn consumer scripts unnecessarily. Re-open trigger: a future library story that actually starts setting `.one` in evidence (which would also require updating the CLAUDE.md type contract).

#### Dismissed (3 — noise, addressed by spec, or NIT-only)

- R1 — `opts.enableTrace = true` override silently overrides user-set `false` (Edge #6 / Blind partial) — **DD #1 explicitly authorizes** the override as the entire premise of Story 5-4's always-on contract. Not a defect.
- R2 — `humanize(_:)` inconsistency: `dedup`/`quorum`/`average` render as single words while `weightedAverage` becomes `"weighted average"` (Blind NIT) — caption-line cosmetic; no test pins the strings. Dismissed as NIT.
- R3 — `cancelInFlight()` lost its "Cancel" branch instruction comments; per-line ADR/DD/PSI comment trail dropped throughout `AnalysisViewModel.swift` (Auditor NIT) — comment-density preference, not a behavior change. Dismissed as NIT.

**Triage totals:** 6 decision-needed / 9 patch / 5 defer / 3 dismissed (from 59 raw findings across 3 layers).
