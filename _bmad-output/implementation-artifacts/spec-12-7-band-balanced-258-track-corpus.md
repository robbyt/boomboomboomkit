...
final_revision: '5524a1b' # PR #197 round-2 rework (2026-08-11); round 1 was c2865b7, original run 202d60a
...
### 2026-08-11 -- PR #197 round 2 (Copilot second pass, verified, then Codex pressure-test x3)

10 findings, all upheld. 0 rejected. **8 of the 10 are defects the 2026-08-10
round introduced.** That round hardened the commitment chain and, doing so,
built a state machine whose paths it never walked; two of its regressions were
outright blockers and two more were policies the committed artifacts advertised
as implemented that did nothing. 1 operator clarification obtained.

- intent_gap: 0
- bad_spec: 0
- patch: 10 (high 4, medium-high 3, medium 3)
- defer: 0
- reject: 0
- regressions_from_the_prior_round: 8
- operator_input_obtained: 1 (the definition of a re-pass "disagreement", recorded
  as a dated clarification in the binding 12-6 document, not as a change to a
  bound item)
- addressed_findings:
  - `[high]` `[patch]` **A. The harness could never mint again.** `commit-pools` halted unconditionally on a superseded record; its prerequisite checks changed only the message, printing "present" for each satisfied gate under the headline "blocked on the following, all of which must be on record first" and then halting anyway, so satisfying every stated gate failed identically to satisfying none. Nothing ever wrote `active` over a superseded record, while both committed artifacts described a re-mint as the next action. An explicit `remint` operation now exists, ordered for crash-safety (validate, archive the predecessor byte-for-byte and verify the archive, build the new generation, replace the commitment last), recording `supersedes_sha256`; commitment validation walks the whole chain so an archived predecessor cannot vanish. It refuses unless the prior generation has NO annotation state. `commit-pools` now lists only genuinely-missing prerequisites.
  - `[high]` `[patch]` **B. The mandatory fingerprint route failed open** in four independent ways (missing audio skipped by a bare `continue`; every decode failure absorbed by `if vec:`; a missing librosa returning None for every file, zeroing the route; an empty training set short-circuiting the completeness check), then recorded `"flags": 0` with no denominators. Now fail-closed and reason-stratified, with the two sides treated asymmetrically: an unfingerprintable CANDIDATE is hard-excluded (it cannot be staged safely anyway, and content binding now runs first), an unfingerprintable TRAINING ROW blocks certification and is unblocked only by restoring audio, correcting the manifest, or a dated operator amendment. A missing decode backend raises immediately. `corpus_common.compute_fingerprint_with_reason` is the single decode implementation supplying the stratification.
  - `[medium]` `[patch]` **C. Confirmed cross-band pairs did not form transitive groups.** Per-flag assignment with no merge made A-B plus B-C yield `A -> g1, B -> g2, C -> g2`, so A and B both survived a confirmation that they are the same recording, with the winner decided by sha256 sort order. Union-find over confirmed edges, operator-supplied ids validated for consistency within a component, and `candidate_key`/`peer_key` surfaced in the disposition template so a human has the key material the broken default had made unnecessary.
  - `[medium-high]` `[patch]` **D. The ledger recorded a digest it never verified.** `work_order_sha256` was written and read nowhere, which is worse than recording nothing because it reads as assurance: a worklist could be re-pointed at different audio, producing labels for the wrong track, while every check passed. Verifying the ingest-recorded digest would still be too late (an alteration BEFORE ingest anchors its own digest as the reference), so the worklist is now validated structurally before annotations are accepted - full row set in work order, every path confined to that batch's staging directory, every staged file hashed against the row's committed `contentSha256` - and the recorded digest is re-checked by the audit for post-ingest edits.
  - `[medium-high]` `[patch]` **E. Two destructive paths lost irreplaceable labels.** A forced re-ingest `os.replace`d the unversioned annotation record, leaving only a digest of bytes that no longer existed; `abandon --force` unlinked the record outright. Both contradicted the committed claim that prior records are never overwritten. Annotation records are now immutable and versioned, each event carries its own record path, and every consumer reads that path instead of reconstructing a hard-coded one. `validate_state` enforces allowed roots, unique paths, digests, and valid transitions.
  - `[medium]` `[patch]` **F. Both operator policies contained a no-op.** `audit-fails-on-cross-band-duplicate` compared only identity, byte digest and normalized title - and a confirmed re-encode differs in identity AND bytes by definition - so the check its name promises did not exist; confirmed recording groups are now persisted at mint and the audit fails on two members sharing one. `fallback-addendum` was accepted but only `shrink-corpus` changed behaviour; the addendum is now enumerated under either policy and committed as a dated artifact with its own appended draw sequence. It honestly reports a net-new set of ZERO for every band, which is exactly what implementing it faithfully was for.
  - `[high]` `[patch]` **G. Signoff skipped a binding signed obligation.** The 2026-08-08 signoff adopted a 10 percent blind re-pass (12-6 lines 484-492); grepping the harness for "re-pass", "disagreement" or "repeatability" returned nothing, and `12-7-eval-corpus.md` never acknowledged it, so a corpus could receive the training-enabling attestation without the mitigation the operator signed. Implemented with an independent domain-separated draw over a separate permutation, an immutable record written before any re-pass annotation, fresh aliases and an independently randomized order, its own state machine (never feeding membership, never replacing a primary annotation, never appending an ordinary batch event), and the operator's two-tier metric plus the continuous difference distribution. Signoff refuses without it.
  - `[medium-high]` `[patch]` **H. The training gate trusted copied digest fields.** `validate_attestation` compared the attestation's ledger-head STRING against the tracked head JSON: it never recomputed the actual head from the event store, never verified the head artifact's commitment binding, and would not have hashed the re-pass record. It now reads and hashes every referenced artifact, and the gate has automated tests against five attestation states.
  - `[structural]` `[patch]` Command functions held planning, validation and mutation at once, so no single place could assert the state machine was coherent - the reason eight regressions shipped. Three narrow boundaries now: a pure planning layer, an immutable event store with an explicit transition table, and one `validate_state()` before AND after every mutation, with the commitment's generation present in every private path (pools and draws included, not only annotations - preserving an archived commitment's JSON leaves its chain unverifiable if the canonical row-level files can be replaced in place).
  - `[test]` `[patch]` The suite gains the lifecycle walk that would have caught A, E and G before review: superseded -> archived -> re-minted -> staged -> ingested -> re-passed -> signed, on a synthetic corpus in `tmp_path`, plus a regression per finding. 173 tests in this file (was 100).
- operator_clarification:
  - The signed protocol requires recording "the disagreement rate" but never
    defined a disagreement. The operator chose two-tier on 2026-08-11:
    metrical-level (an octave-RATIO criterion, within 4 percent of 2x or 0.5x)
    counted separately from fine (same level, above 0.5 BPM), with
    `non-comparable` reserved for no usable numeric result and `crossed_band` as
    an additional flag rather than a category. Recorded as a dated clarification
    in 12-6, which is the binding document. No pass/fail threshold: the signed
    text records the rate, and a gate would be a new bound item needing its own
    signature.
