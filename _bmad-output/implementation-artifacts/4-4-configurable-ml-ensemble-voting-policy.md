...
- [x] **Task 1: Pre-source baseline capture (commit pre-source artifacts BEFORE first source edit) (AC: #4, #5)**
  ...
  - [x] 1.4: Commit Task 1 artifacts as a single pre-source commit: `Story 4-4 Task 1: pre-source-change baseline artifacts` (mirrors Story 4-3's Task 1 pattern, commit `9fd7c44`). This commit MUST land BEFORE any source edit so the snapshot represents pre-Story-4.4 behavior.
...
- [x] **Task 8: Standard gating checklist + Completion Notes (AC: #4, #9, #12)**
  ...
  - [x] 8.13: Commit the source changes + tests + Makefile + artifacts as a single commit: `Story 4-4: configurable EnsemblePolicy + EnsembleCombiner promotion` (mirrors Story 4-3 commit `c1ba272 Story 4-3: wire MLTechnique slot, migrate tuples to MLEvaluation struct`).
...
- **Gating-checklist outcomes (Story 4.3 final):** `make benchmark` OA300 Acc1=58/82, Acc2=74/82; `make benchmark-giantsteps` Acc1=537/661, Acc2=546/661; `make ablation` `.optimal` Acc1=55/82; `make perf-benchmark` mock-on-abstain ratio 1.092x. Story 4-3b tightened the perf-gate threshold to 1.20x (commit `aa786e3`).
...
From Story 4-3b (commit `aa786e3`, completed 2026-05-06):
...
- `aa786e3` Story 4-3b: perf-gate threshold tightening + `subBandEnergies` typed migration. Story 4.4 inherits the 1.20x perf threshold.
- `c1ba272` Story 4-3: ML technique slot wiring + tuple-to-struct migration. Story 4.4 promotes the `combine` helper this story shipped.
- `9fd7c44` Story 4-3 Task 1: pre-source-change baseline artifacts. **Pattern reuse:** Story 4.4 Task 1 follows the same convention.
- `9185698` Story 4.2: report effective intensity and graceful ML degradation. Story 4.4 does not touch `effectiveIntensity` or `degradationReason` (orthogonal — DD #10).
- `29ced70` Story 4.1: scaffold `BoomBoomBoomKitML` SPM target. Untouched by 4.4.
...
| `make perf-benchmark` mock-on-abstain ratio | **0.992x** at SHA `16e03e0` (2026-05-09 re-run); was **1.000x** at SHA `a594079` (post-A1 close-out), **1.093x** at Task 1 baseline (SHA `aa786e3`). Δ vs prior baseline (`be38d80`): mean 0.177s → 0.176s (-0.4%) | `_bmad-output/perf-baselines/Apple_M5_Max-26--Debug--20260509T011243Z--16e03e0--e3dbd9f1.json` |
...
**New (artifacts):**
- `_bmad-output/implementation-artifacts/4-4-regression-snapshot.json` (Task 1 commit) — informational snapshot of OA300/GiantSteps Acc1/Acc2 + failure subset at SHA `aa786e3`.
...
- `_bmad-output/perf-baselines/Apple_M5_Max-26--Debug--20260507T062546Z--aa786e3--31de1a20.json` (Task 1 commit) — pre-source perf snapshot at the Task 1 SHA.
