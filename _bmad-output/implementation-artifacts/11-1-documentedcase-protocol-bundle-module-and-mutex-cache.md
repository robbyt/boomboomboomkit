...
- **Gating (final, all green):** `make fmt` clean; `make lint` `Found 5 violations, 0 serious in 176 files` — all 5 pre-existing `Demo/`-only (zero in the new source/test files, grep-confirmed); `make build` "Build complete!"; `make test` = **853 tests / 140 suites passed** (the `Story 11.1 DocumentedCase` suite contributes 6 tests: shape, happy-path, fallback, associated-value-inherits-docs, blank-parse-guard, concurrent-safety). Orchestrator-verified.
