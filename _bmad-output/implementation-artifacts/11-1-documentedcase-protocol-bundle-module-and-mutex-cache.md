...
- **Gating (final, all green):** `make fmt` clean; `make lint` `Found 5 violations, 0 serious in 176 files` — all 5 pre-existing `Demo/`-only (zero in the new source/test files, grep-confirmed); `make build` "Build complete!"; `make test` = **852 tests / 140 suites passed** (the `Story 11.1 DocumentedCase` suite contributes 6 tests: shape, happy-path, fallback, associated-value-inherits-docs, blank-parse-guard, concurrent-safety). Orchestrator-verified.
...
- `Tests/BoomBoomBoomKitTests/DocumentedCaseTests.swift` (new) — 6 tests (shape, happy-path, fallback, associated-value-inherits-docs, blank-parse-guard, concurrent-safety).
...
- `make fmt` clean; `make lint` 5 pre-existing `Demo/`-only violations (0 in new files, grep-confirmed); `make build` "Build complete!"; `make test` **852 tests / 140 suites passed**. Gates re-run and confirmed by the orchestrator after every material change (pass 1, re-derive, iteration-2 patches).
