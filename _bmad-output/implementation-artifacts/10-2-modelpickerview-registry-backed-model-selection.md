...
It is the demo model-picker sheet + a demo-side catalog wrapping the library `ModelRegistry` and the 10.1 `BookmarkPersistence`. It is NOT a change to the library registry, NOT a bundled-model addition, and NOT ML BYOW infrastructure (that already ships — `attachMLTechnique`/`BNNSTechnique`). It reorganizes the existing single-model BYOW load into a persistent, multi-entry picker.

**Selection is session-scoped by design (AC5 scope).** `restore()` re-lists persisted user-added models across launches, but does NOT re-attach a `BNNSTechnique` — `selectedURL` is `nil` after relaunch, so a previously-chosen model appears in the picker but is not loaded until the user re-picks "Use this model". AC5 promises list presence only. Persisting the selection + auto-loading on launch is the follow-up **W81** (its own small story; carries the DD5 main-actor `BNNSTechnique`-compile caveat).
...
- 2026-07-10 — PR #92 review triage (2 Codex passes: scope consult `019f4a82`, plan review `019f4a85`). 6 inline threads: 2 dismiss + 4 patched. Dismissed both Copilot scope-mismatch claims (start-scope-on-`url`-read-standardized-`key` is correct; their fix — scope on `key` — returns false; added clarity comments). Patches: R1 `clearRestoreDiagnostics()` cleared on `ModelPickerView.onDisappear` + regression test; R2 documented session-scoped selection (Dev Notes + `selectedURL` doc + W81 deferred); R3 hoisted `ModelCatalog` to `BoomBoomBoomBPMApp` (was a `ContentView` `@State` re-running restore each construction) + fixed the ownership comment; R4a deleted the dead `pickAndLoadMLModel()`; R4b made `BookmarkPersistence.withSecurityScopedAccess` `static` + dropped `AnalysisViewModel`'s injected `bookmarkPersistence`; R4c dropped the duplicate empty-state `Reason: no-models-available`. Deferred W82 (W79's persistence-layer half). Demo gauntlet green; `git diff Sources/ Tests/` empty.
