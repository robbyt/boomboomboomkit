...
- [x] [Review][Defer] Python degenerate-input hardening in the decomposition spike [decompose_beatgrid.py:174-178,186-195,134/233] — deferred, near-unreachable on the real artifacts: empty `p95s` (all constant oracles <3 beats) → `np.median([])` NaN; empty `rows` (mismatched `--accuracy`/`--reference`) → `np.mean([])` NaN; explicit `"basename": null` → `r['basename'][:40]` TypeError. The spike already ran successfully on the full corpus.
