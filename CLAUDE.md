...
- **EnsemblePolicy** — Public enum (`String, CaseIterable, Sendable, Hashable`, 3 cases): `.dspOnly` (default — `MLTechnique.evaluate` is operation-inert AND output-inert short-circuited), `.mlOnly` (ML wins when `evaluation != nil`; falls through to DSP otherwise), `.highestConfidence` (winner is whichever side self-reports higher `confidence`, deterministic DSP tiebreak). Carried on `AudioAnalysisService.Options.ensemblePolicy`. NOT a `DSPTechnique` (no DSP changes, no ablation-matrix expansion); NOT a `BPMSelectionPolicy` case (still 8). The `.dspOnly` byte-identity contract under non-nil `mlTechnique` is test-locked by `EnsemblePolicyTests::dspOnlyDoesNotInvokeMLTechnique` (Story 4-4 AC #14).
...
- **MetadataCorroborator** — Internal caseless enum namespace. `apply(to:input:)` runs AFTER `BPMSelectionPolicy.merge` returns: boosts every candidate matching any participating tag, re-selects the winner from the boosted pool (allows tag-driven winner-promotion), and applies confidence boost (×1.25 clamp 0.95) or skepticism penalty (×0.85). Lives outside `merge` to keep that signature stable across 41 call sites and to ensure single-window paths (intensity 1-5) still apply corroboration. The internal `MetadataCorroborationInput` struct (also in this file) carries the consensus signal from the service to `apply`.
